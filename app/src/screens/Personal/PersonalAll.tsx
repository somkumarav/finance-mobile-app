...
const Modal: React.FC<Modal> = ({ setShowModal }) => {
  ...
  const { addData } = useContext(PersonalContext);
...
export const PersonalAll = ({
  navigation,
}: {
  navigation: NativeStackNavigationProp<PersonalParamList, 'All'>;
}) => {
  const [showModal, setShowModal] = useState<boolean>(false);
  const { allData } = useContext(PersonalContext);

  return (
    <View style={styles.personalAll}>
      {showModal && <Modal setShowModal={setShowModal} />}
      <PersonalHeader setShowModal={setShowModal} />
      <PersonalMain allData={allData} navigation={navigation} />
    </View>
  );
};
